...
function f_and_jac(fx, jac, u_and_t)
    u = @view u_and_t[1:p]
...
jac = vcat(zeros(2*p)', ∇cons)
fx = zeros(2p + 1)

f_and_jac(fx, jac, u_and_t)
fx[1]
sum((G * u - y).^2) + α * sum(t)
...
opt = init(f_and_jac, lb, ub, 2p, 2p; x0=u_and_t, max_iters=2, max_inner_iters=10, max_dual_iters=5, max_dual_inner_iters=5, jac_prototype=jac);
sol = solve!(opt)
