...
f_and_∇f(fx, ∇fx, zeros(n))
iterate = SparseCCSA.init_iterate(; n, m, x0 = zeros(2), ∇fx_prototype = ∇fx, lb=[-Inf, -Inf], ub=[Inf, Inf])    
iterate.fx .= fx
...
opt = init(f_and_∇f, fill(typemin(0.0), 2), fill(typemax(0.0), 2), n, m; x0 = [0.0, 0.0], max_iters = 5,
           max_inner_iters = 5,
